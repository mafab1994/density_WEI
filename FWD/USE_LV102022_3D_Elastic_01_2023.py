...
vel_model_type = 'heterogeneous_xy_hetZ'#'homogeneous'#'het_z'#'heterogeneous_xy_hetZ'#'het_z'#'heterogeneous_xy_hetZ'#'homogeneous'#"heterogeneous_xy_hetZ" #"homogeneous"#"heterogeneous_xy_hetZ" #"heterogeneous_xy_hetZ"

# Chose between velocity models (1) "homogeneous" (2) "linear_grad" (3) "het_z" (4) "sin_dist" (5) "heterogeneous_xy"
rho_model_type = 'SAME'#'homogeneous'#'het_z'#"sin_dist2b_hetZ"# 'het_z' #"sin_dist2b_hetZ"#'het_z'#"sin_dist2b_hetZ"#'test' # "sin_dist2b_hetZ"#"sin_dist2b_hetZ" #"sin_dist2"#"sin_dist2"
...
lam = ((vp**2) * rho) - (2 * (vs**2) * rho)
mu = ( (vs**2) * rho)

# +
#for ii in range(0,rho.shape[2]):
#    rho[:,:,ii] = rho[:,:,ii].T
...
plt.imshow(ds_without_q.vp[261:339,261:339,-1])#, origin='lower')
plt.colorbar()
plt.show()
# -

plt.plot(ds_without_q.rho[261:339,300,-1])
plt.plot(ds_without_q.rho[21:99,300,-1])
...
# +
#plt.plot(ds_without_q.mu[261:339,300,-1])
#plt.plot(ds_without_q.mu[21:99,300,-1])
#plt.plot(ds_without_q.lam[300,261:339,-1])
#np.mean(mu)
# -

plt.figure(figsize=(10, 8))
#ds_without_q.rho[298:308,298:308,0].plot()
plt.imshow(ds_without_q.rho[:,:,-1])#, origin='lower')
plt.colorbar()
plt.show()
...
#plt.figure(figsize=(10, 8))
#ds_without_q.rho[298:308,298:308,0].plot()
#plt.imshow(ds_without_q.lam[:,:,-1])#, origin='lower')
#plt.colorbar()
#plt.show()
# -
...
project_name = '3DELASTIC_HET600x300_HET_abs2_4el_test2_hetZ_AMBIENTii_LARGER-8el_SAME'#'3Delastic_HOM_Q40'#MODEL2'#VISCO_Q8'#40_1mXY'#'3DACOUSTIC_HOM600x300_HOM_ambient16el'
...
# Or create a new one.
    ...
    p = sn.Project.from_domain(
        ...
        domain=sn.domain.dim3.BoxDomain(x0=0, x1=nx, y0=0, y1=ny, z0=0, z1=nz),
    )
    path_ev = str(output)+"out/"+str(project_name)+"/EVENTS/"+str(event_name)
    os.mkdir(path_ev)
    fig_ = str(path_ev)+'/FIGURES'
...
# +
source_type = 'ambient_noise1'                  # 'active' or 'ambient_noise1'
fx,fy, fz= 1e3,1e3,1e3 #1e3, 1e3                # Source components (Nm)
T = 1/f_max
...
#     dl = np.array([1.2, 3.0, 1.6, 1.4, 1.7, 2.0, 2.5, 1.8, 0.6, 1.3])
    ...
    src = [ sc.source.cartesian.VectorPoint3D(x=_x, y=_y, z=sz, fx=fx,fy=fy,fz=fz) for (_x,_y) in loc]
...
#     wavelets = [stf1,stf2,stf3,stf4,stf5,stf6,stf7,stf8,stf9,stf10]
    ...
    sx, sy, sz = ((nx)/2)+1.5, ((ny)/2)+1.5, nz - source_depth                  # Source position (m)
    src = sc.source.cartesian.VectorPoint3D(x=sx, y=sy,z=sz, fx=fx,fy=fy, fz=fz)

    stf8 = sc.source.stf.Delta()
    wavelets = [stf8]
    ...
    c_f = 12.0
    dl = 2
    source_depth = nz-nz+(nz-290) #shallow#nz-nz+190 #nz-5 #nz-nz+5
    source_depth = 290 #deep
...
elif source_type == 'active_isotropic':
    c_f = 12.0
    dl = 2
    source_depth = nz-nz+(nz-290)#nz-nz+190 #nz-5 #nz-nz+5
...
recs = [
    sn.simple_config.receiver.cartesian.Point3D(
    #    x=x, y=y, z=nz, station_code=f"{_i:03d}{_j:03d}", fields=["velocity"]
    x=x, y=y, z=z ,station_code=f"{_i:02d}{_j:02d}{_k:02d}", fields=["displacement","velocity","gradient-of-displacement","acceleration"] #['stress','stress-tensor']
    )
    for _i, x in enumerate(np.linspace(rx0, rx1, nr))
    for _j, y in enumerate(np.linspace(ry0, ry1, nr))
    for _k, z in enumerate(np.linspace(rz0, rz1, nrz))
...
for attenuation in [True, False]:
    ...
    max_frequency_in_hertz= 2*np.max(c_f[0:5]),#[0:5]),#24,#2*np.max(c_f[6:10]),#center_frequency * 2.0,# np.max(c_f[0:5])*2,#center_frequency * 2.0,#np.max(c_f[0:5])*2, #f_max, #center_frequency * 2.0,
    tensor_order = 1,
    elements_per_wavelength=2.0,
    model_configuration=mc,
    absorbing_boundaries = mbm.AbsorbingBoundaryParameters(reference_velocity=np.min(vs), free_surface = True,
                                                    number_of_wavelengths=1.5, reference_frequency=16),#(center_frequency * 2.0)
    ...
    #time_step_in_seconds = 0.0001574012757931994, #0.0001,#0.00033912678552380765# 0.0001,# 2.115205e-04,#0.00023171707720226795/3, #/5,
...
p.simulations.get_simulation_output_directory('without_attenuation', event_name)
...
# +
#test= p.simulations.get_mesh("without_attenuation")

# +
#test
# -

mesh = p.viz.nb.simulation_setup("with_attenuation")
#mesh
...
mesh.elemental_fields['fluid']= np.zeros(mesh.elemental_fields['fluid'].shape)
...
output_grid = extract_model_to_regular_grid(
    mesh=mesh, ds=ds, pars=["VP","VS","RHO"], verbose=True 
)

# #["VP","VS","RHO","QKAPPA","QMU"]
# -

output_grid.RHO.sel(x=261).T.plot()

# +
#input_test='/exports/csce/datastore/geos/users/s2110831/out/3DELASTIC_HOM600x300_HET_abs2_4el_test2_hetZ_AMBIENTii_LARGER-/EVENTS/src_loc_c_isotropic_FS_src_SURF/INPUT_MODELS'

# +
#output_grid = xr.open_dataset(str(input_test)+"/model_true.nc")
# -

output_grid

plt.imshow(ds_without_q.rho[:,:,-1]-output_grid.RHO[:,:,-1])
plt.colorbar()

plt.plot(ds_without_q.rho[260:338,50,-1])
plt.plot(output_grid.RHO[260:338,50,-1])

plt.imshow(ds_without_q.rho[:,:,-1])

plt.imshow(output_grid.RHO[:,:,-1])

# +
# from scipy.io import netcdf
# import netCDF4
# path_test= '/exports/csce/datastore/geos/users/s2110831/out/3DELASTIC_HET600x300_HET_abs2_4el_test2_hetZ_AMBIENTii_LARGER-/EVENTS/src_loc_c_isotropic_FS_src_SURF/INPUT_MODELS'
# fp=str(path_test)+"/model_true.nc"
# nc = netCDF4.Dataset(fp)
# temp = nc.variables['RHO']
# #nc = netCDF4.Dataset(fp)
# data = temp[:]*1
...
output_grid.RHO.values == ds.RHO.values

# #output_grid
ds.to_netcdf(str(input_)+"/model_true.nc", engine="h5netcdf", mode='a')

# +
#dsRHO =dsRHO_sub[261:339,261:339,-1]

# +
#plt.imshow(dsRHO[::2,::2],origin='lower')#.plot()
#plt.colorbar()
# #ds.VP[0,0,140:155].plot()

# +
#plt.plot(dsRHO_sub[261:339,261:339,-1].T.flatten())
...
mesh.element_nodal_fields['RHO'].shape#[:,0]#[66265:66275,0]
...
#for name in ["with_attenuation", "without_attenuation"]:
for name in ["without_attenuation"]:
...
for jj in range(0,len(EV_att.receivers)):
    ...
    disp_gradXx_att = EV_att.get_receiver_data(EV.receiver_name_list[jj], receiver_field='gradient-of-displacement')[0]
    disp_gradXy_att = EV_att.get_receiver_data(EV.receiver_name_list[jj], receiver_field='gradient-of-displacement')[1]
    disp_gradXz_att = EV_att.get_receiver_data(EV.receiver_name_list[jj], receiver_field='gradient-of-displacement')[2]
    disp_gradYx_att = EV_att.get_receiver_data(EV.receiver_name_list[jj], receiver_field='gradient-of-displacement')[3]
    disp_gradYy_att = EV_att.get_receiver_data(EV.receiver_name_list[jj], receiver_field='gradient-of-displacement')[4]
    disp_gradYz_att = EV_att.get_receiver_data(EV.receiver_name_list[jj], receiver_field='gradient-of-displacement')[5]
    disp_gradZx_att = EV_att.get_receiver_data(EV.receiver_name_list[jj], receiver_field='gradient-of-displacement')[6]
    disp_gradZy_att = EV_att.get_receiver_data(EV.receiver_name_list[jj], receiver_field='gradient-of-displacement')[7]
    disp_gradZz_att = EV_att.get_receiver_data(EV.receiver_name_list[jj], receiver_field='gradient-of-displacement')[8]
...
output_fields.to_netcdf(output_+"/output_ATT.nc", engine="h5netcdf", mode='a')
